...
def query():
    ...
    task_data = {"id": task_id, "hospital_name": hospital_name}

    task_data = json.dumps(task_data)

    # Creating the Redis client to connect to the redis worker queue
    redisClient = redis.StrictRedis(host=redisHost, port=redisPort, db=1)
...
def get_result(task_id):
    # Creating the Redis client to connect to the redis worker queue
    redisClient = redis.StrictRedis(host=redisHost, port=redisPort, db=3)
